...
**STOP: This lab requires you have already followed the instructions in the LabSetup**
...
You should have already completed the prelab to setup RHEL (vm002) in an Azure Virtual Machine and installed an ssh client, and SQL Server Management Studio (Windows clients only).
...
3. Now you must complete the installation by executing a bash shell script we install called **mssql-conf** (which uses a series of python scripts). We will also use mssql-conf later to perform a configuration task for SQL Server. Execute the following command
    ...
    Go through the prompts to pick Edition (choose Developer or Enterprise for these labs), accept the EULA, and put in the sa password (must meet strong password requirements like SQL Server on Windows). Remember the sa password as you will use it often in the labs.
